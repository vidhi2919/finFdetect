...
class GraphBuilderAgent:
    ...
    def push_to_neo4j(self, driver):
        """
        Push current NetworkX graph to Neo4j.
        Nodes: Account IDs
        Edges: Money transfers with attributes
        """
        if self.graph.number_of_nodes() == 0:
            print("Graph is empty. Nothing to push.")
            return
        ...
        with driver.session() as session:
            ...
            for sender, receiver, data in self.graph.edges(data=True):
                session.run(
                    ...
                    SET t.total_amount = $total_amount,
                        t.count = $count,
                        t.first_time = $first_time,
                        t.last_time = $last_time,
                        t.avg_amount = $avg_amount,
                        t.transaction_ids = $transaction_ids
                    """,
                    sender=sender,
                    receiver=receiver,
                    total_amount=data['total_amount'],
                    count=data['count'],
                    first_time=str(data['first_time']),
                    last_time=str(data['last_time']),
                    avg_amount=data['avg_amount'],
                    transaction_ids=data['transaction_ids']
                )
        print(f"✓ Graph pushed to Neo4j: {self.graph.number_of_nodes()} nodes, {self.graph.number_of_edges()} edges")
...
if __name__ == "__main__":
    ...
    # 1. Load Neo4j credentials from environment variables
    uri = "neo4j+s://1bd6c7c1.databases.neo4j.io"
    username = "neo4j"
    password = "dMjDwAH1eaFedHCnpZAUT-pM3zR5GQc0Uhz3utsPBFU"

    # 2. Create Neo4j driver (this is the missing piece)
